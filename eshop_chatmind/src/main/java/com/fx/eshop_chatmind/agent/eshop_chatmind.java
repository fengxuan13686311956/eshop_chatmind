...
import com.fx.eshop_chatmind.model.vo.ChatMessageVO;
...
public class eshop_chatmind {
    ...
    public eshop_chatmind() {
    }

    public eshop_chatmind(String agentId,
                     String name,
                     String description,
                     String systemPrompt,
    ...
    private boolean think() {
        String thinkPrompt = """
                现在你是一个智能的「决策模块」，请根据当前对话上下文决定下一步动作。

                【核心规则 —— 必须严格遵守】
                1. 你的内部知识可能已过时，**严禁在未查询外部数据源的情况下直接回答事实性问题**。
                2. 对于需要数据支撑的问题，必须优先调用 getTableSchema / databaseQuery（数据库查询）或 KnowledgeTool（知识库检索），获取最新数据后再作答。
                3. 如果用户问题涉及具体数据、事实、记录，你必须先调用工具获取真实数据，而不是凭记忆回答。
                4. 只有当问题纯属闲聊、常识推理且不依赖具体数据时，才可以直接回答。

                【可用数据源】
                - 知识库列表：%s
                - 数据库查询工具：getTableSchema（获取表结构）→ databaseQuery（执行SQL）
                - 知识库检索工具：KnowledgeTool（混合检索：语义+关键词）

                【推荐流程】
                第一步：分析用户问题，判断是否涉及数据查询
                第二步：若涉及 → 先调用 getTableSchema 了解数据结构，或调用 KnowledgeTool 检索知识库
                第三步：基于工具返回的真实数据生成回答
