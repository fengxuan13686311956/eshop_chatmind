...
import com.fx.eshop_chatmind.service.DocumentFacadeService;
...
public class DocumentFacadeServiceImpl implements DocumentFacadeService {
    ...
    public CreateDocumentResponse uploadDocument(String kbId, MultipartFile file) {
        try {
            ...
            // 计算文件内容 Hash
            String contentHash = computeFileHash(documentStorageService.getFilePath(filePath).toString());

            // 更新文档记录，保存文件路径和内容Hash到 metadata
     ...
     * 计算文件内容 SHA-256 Hash
     */
    private String computeFileHash(String absolutePath) {
        try {
            ...
            log.error("计算文件 Hash 失败: {}", absolutePath, e);
            return "";
        }
    }
